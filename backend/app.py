...
def analyze_dataset():
    try:
        ...
        try:
            ...
            report_generator = ReportGenerator(analysis_report, health_score)
        ...
        print(f"Keys: {list(full_response.keys())}", file=sys.stderr)
        print(f"Success: {full_response['success']}", file=sys.stderr)
        print(f"Health Score: {full_response['health_score']}", file=sys.stderr)
        print(
            f"Verdict Status: {full_response.get('verdict', {}).get('status')}",
            file=sys.stderr
        )
        print(f"Top Risks Count: {len(full_response.get('top_risks', []))}", file=sys.stderr)

        response_str = json.dumps(full_response)
        print(f"Response size: {len(response_str)} chars", file=sys.stderr)
        print(f"Response size: {len(response_str.encode())} bytes", file=sys.stderr)
        print("✅ JSON serializable", file=sys.stderr)
        print("=" * 60 + "\n", file=sys.stderr)
        # ===================================================
